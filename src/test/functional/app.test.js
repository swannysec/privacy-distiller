/**
 * Functional tests for Privacy Policy Analyzer
...
test.describe('Privacy Policy Analyzer - Dev Server', () => {
  ...
  test('should load the application homepage', async ({ page }) => {
    ...
    // Verify page title
    await expect(page).toHaveTitle(/Privacy Policy Analyzer/i);
    
    // Verify main heading
    const heading = page.locator('h1');
    await expect(heading).toBeVisible();
    await expect(heading).toContainText(/Privacy Policy Analyzer/i);
  });
  ...
  test('should display LLM configuration panel', async ({ page }) => {
    ...
    const providerLabel = page.locator('text=/Provider/i');
...
test.describe('Privacy Policy Analyzer - Preview Server', () => {
...
test.describe('Privacy Policy Analyzer - User Workflows', () => {
  test.use({ baseURL: DEV_URL });
  ...
  test('should display validation errors for invalid URL', async ({ page }) => {
    ...
    if (await urlInput.isVisible()) {
      ...
      if (await analyzeButton.isVisible()) {
        ...
        const errorMessage = page.locator('text=/invalid/i, text=/error/i, [role="alert"]');
  ...
  test('should require API key for analysis', async ({ page }) => {
    ...
    // Try to analyze without API key
    const analyzeButton = page.locator('button:has-text("Analyze"), button:has-text("Submit")').first();
    
    if (await analyzeButton.isVisible()) {
      await analyzeButton.click();
      
      // Should show some indication that configuration is needed
      await page.waitForTimeout(500);
      
      // The app should handle missing configuration gracefully
      const body = page.locator('body');
      await expect(body).toBeVisible();
    }
  });
...
test.describe('Privacy Policy Analyzer - Accessibility', () => {
  test.use({ baseURL: DEV_URL });
  ...
  test('should have ARIA labels on interactive elements', async ({ page }) => {
    ...
    const labeledInputCount = await inputs.count();
...
test.describe('Privacy Policy Analyzer - Performance', () => {
  test.use({ baseURL: PREVIEW_URL });
